...
> monorepo as `apps/indexstr`. The crawler engine is the shared
> `@sip01/crawler-core` package and the SIP-01 wire format is
> `@sip01/protocol` — this repo holds only the network-node UI, the curated
> collections + SQLite reader, and app policy. See
> [MIGRATION.md](../../MIGRATION.md) for v1 → v2 notes. Build/test from the
> monorepo root with `pnpm -r test` / `pnpm -r build`.
